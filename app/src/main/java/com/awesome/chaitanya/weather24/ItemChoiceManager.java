...
/**
 * This class keeps track of which positions have been selected.
 * Note that it doesn't take advantage of new adapter features to track changes in the
 * underlying data.
 */
public class ItemChoiceManager {
    ...
    /**
     * how many positions in either direction we will search to try to find a checked item
     * with a stable ID that moved a position across a data set change. If the item isn't
     * found it will be unselected
     */
    ...
    /**
     * Running state of which IDs are currently checked. If there is a value for a
     * given key, the checked state for that ID is true and valure holds the last
    ...
    public void onClick(RecyclerView.ViewHolder vh) {
        ...
        switch (mChoiceMode) {
            ...
            case AbsListView.CHOICE_MODE_SINGLE: {
                ...
                /**
                 * We directly call onBindViewHolder here because notifying that an item has changed
                 * on an item that has the focus causes it to lose focus, which makes keyboard
    ...
    /**
     * Define the choice behavior for the Recyclerview. By default, RecyclerViewChoiceMode does not
     * have any choice behavior.
     */
     ...
     * mode has been set to AbsListView.CHOICE_MODE_SINGLE, but the code does not check this
     */
    ...
    void confirmCheckedPositionsById(int oldItemCount) {
        ...
        //clear out the positional check status, we'll rebuild it below from IDs.
        mCheckStates.clear();
